;;; rect.el --- rectangle functions for GNU Emacs  -*- Mode: Lisp;-*- 
...
#|(defvar-local mark-crutches nil
  "(POS . COL) to override the column to use for the mark.")|#


(defun pos-cols (start end &optional window)
  (declare (ignore window))
  (when (point> start end)
    (rotatef start end))
  (cons (point-column start)
        (point-column end)))


#|(defun col-pos (col kind)
  (let ((c (move-to-column col)))
    (if (and (= c col) (not (eolp)))
        (if (eq kind 'point)
            (if (window-parameter nil 'point-crutches)
                (setf (window-parameter nil 'point-crutches) nil))
          (if mark-crutches (setq mark-crutches nil)))
      ;; If move-to-column overshot, move back one char so we're
      ;; at the position where highlight-for-redisplay
      ;; will add the overlay (so that the cursor can be drawn at the
      ;; right place).
      (when (> c col) (forward-char -1))
      (setf (if (eq kind 'point)
                (window-parameter nil 'point-crutches)
              mark-crutches)
            (cons (point) col)))))|#

#|(defun point-col (pos)
  (let ((pc (window-parameter nil 'point-crutches)))
...
#|(defun reset-point-crutches ()
  (if (window-parameter nil 'point-crutches)
      (setf (window-parameter nil 'point-crutches) nil)))|#

#|(defun reset-crutches ()
  (kill-local-variable 'mark-crutches)
  (reset-point-crutches))|#
...
(defun delete-rectangle-line (startcol endcol fill)
  (when (= (move-to-column startcol (if fill t 'coerce)) startcol)
    (with-point ((beg (current-point)))
      (ed.call-delete-points-and-save beg
...
(defun delete-extract-rectangle-line (startcol endcol lines fill)
  (let ((pt (line-end-position)))
	...
	(#|then|#
        ...
        (#|else|# 
         progn
          (setq pt (copy-point (current-point)))
          (move-to-column endcol t)
          (setf (cdr lines)
                (cons (points-to-string pt (current-point))
                      (cdr lines)))
...
to be deleted."
  ...
  (declare (ignore p))
  (save-excursion
    (with-point ((start (current-mark) :after-insert)
                 (end (current-point) :after-insert))
      (apply-on-rectangle (lambda (startcol endcol &rest args)
                            (declare (ignore args))
...
#|(defvar-local string-preview-state nil)|#
#|(defvar-local string-preview-window nil)|#

#|(defun string-flush-preview ()
  (mapc #'delete-overlay (nthcdr 3 string-preview-state))
  (setf (nthcdr 3 string-preview-state) nil))|#

#|(defun string-erase-preview ()
  (with-selected-window string-preview-window
    (string-flush-preview)))|#
...
#|(defun string-preview ()
  (when rectangle-preview
    (let ((str (minibuffer-contents)))
      (when str (setq str (propertize str 'face 'rectangle-preview)))
      (with-selected-window string-preview-window
        (unless (or (null string-preview-state)
                    (equal str (car string-preview-state)))
          (string-flush-preview)
          (apply-on-rectangle
           (lambda (startcol endcol)
           ...
           (nth 1 string-preview-state)
           (nth 2 string-preview-state)))))))|#

;; FIXME: Should this be turned into inhibit-region-highlight and made to apply
;; to non-rectangular regions as well?
#|(defvar inhibit-region-highlight nil)|#

;;;###autoload
(defcommand "String Rectangle" (p)
...
Called from a program, takes three args; START, END and STRING."
     "Replace rectangle contents with STRING on each line."
  (declare (ignore p))
  (let ((str (prompt-for-string :prompt "String Rectangle: ")))
    (save-excursion
      (with-point ((start (current-mark) :after-insert)
                   (end (current-point) :after-insert))
        (apply-on-rectangle (lambda (startcol endcol &rest args)
                              (declare (ignore args))
                              (string-rectangle-line startcol endcol str T))
                            start
                            end)))))
...
#|(defun default-line-number-format (start end start-at)
  (concat "%"
...
with a prefix argument, prompt for START-AT and FORMAT."
  ...
  (unless format
    (setq format (default-line-number-format start end start-at)))
...
#|(defun rectangle-exchange-point-and-mark (&optional arg)
  ...
  (if arg
    ...
    (let* ((p (point))
	   ...
	   (m (mark))
           (p<m (< p m))
           (cols (if p<m (pos-cols p m) (pos-cols m p)))
           (cp (if p<m (car cols) (cdr cols)))
           (cm (if p<m (cdr cols) (car cols))))
      (if repeat (setq this-command 'exchange-point-and-mark))
      (reset-crutches)
      (goto-char p)
      (col-pos (if repeat cm cp) 'mark)
      (set-mark (point))
      (goto-char m)
      (col-pos (if repeat cp cm) 'point))))|#

#|(defun *-char (cmd n &optional other-cmd)
...
#|(defun extract-region (orig &optional delete)
  (cond
    ...
    (let* ((strs (funcall (if delete
      ...
      (when strs
        (put-text-property 0 (length str) 'yank-handler
                           `(insert-for-yank ,strs t)
                           str)
        str)))))|#

#|(defun insert-region (orig strings)
  (cond
   ((not rectangle-mark-mode)
...
#|(defun place-cursor (leftcol left str)
  (let ((pc (window-parameter nil 'point-crutches)))
    (if (and (eq left (car pc)) (eq leftcol (cdr pc)))
        (put-text-property 0 1 'cursor 1 str))))|#

#|(defun highlight-for-redisplay (orig start end window rol)
  (cond
   ((not rectangle-mark-mode)
    (funcall orig start end window rol))
   (inhibit-region-highlight
   ...
   (t
    (save-excursion
      (let* ((nrol nil)
        ...
        ;; `pos-cols' looks up the `selected-window's parameter!
             ...
             (let* ((mleft (move-to-column leftcol))
               ...
               (cond
                ((< mleft leftcol)      ;`leftcol' is past EOL.
                 (overlay-put ol 'before-string (space-to leftcol))
...
#|(defun unhighlight-for-redisplay (orig rol)
